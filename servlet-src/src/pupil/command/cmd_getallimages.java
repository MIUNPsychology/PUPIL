...
public class cmd_getallimages extends Command
{
  //private static org.apache.log4j.Logger log = org.apache.log4j.Logger.getLogger(cmd_getallimages.class);
  private static LogWrapper log = new LogWrapper();

  public cmd_getallimages(DBManager dbm, StaticInfoBlob info)
  {
    super(dbm,info);
  }
  ...
  {
    log.trace("Enter cmd_getallimages()");
    ...
    try
    {
      ...
      sql = sql + "category.category_id = image.category_id ";
      sql = sql + "AND category.project_id = ? ";
      sql = sql + "ORDER BY category.name, image.file_name";
    ...
    log.trace("Leave cmd_getallimages()");
