...
public class cmd_remcodepermission extends Command
{
  ...
  {
    log.trace("Enter cmd_remcodepermission()");
    ...
    try
    {
      String sql = "SELECT student.student_id, student.login FROM student WHERE EXISTS ";
      ...
      while(r.next())
      {
        ...
        log.debug("HAS PERMISSION: " + plogin + " (" + pid + ")");
      }

      log.debug("perm size: " + perm.size());
    ...
    try
    {
      ...
      {
        if(perm.contains(alogin))
        {
          stid = studids.get(alogin);
          ps.setString(1,stid);

          log.debug("REMOVING PERMISSION: " + alogin + " (" + stid + ")");

          db.executeAsUpdate(ps,false);
        }
        else
        {
          log.debug(alogin + " did not have permission or did not exist");
        }
      }
      ps.close();
    }
    ...
    log.trace("Leave cmd_remcodepermission()");
