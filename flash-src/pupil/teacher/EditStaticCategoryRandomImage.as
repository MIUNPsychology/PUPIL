package pupil.teacher
{
  ...
  public class EditStaticCategoryRandomImage
  {
    ...
    public function EditStaticCategoryRandomImage(myscreen:SFScreen, parentwin:ProjectDetails, pattern:String, scenename:String, projname:String)
    {
      screen = myscreen;
      myparent = parentwin;
      ...
      for(i = 0; i < myMaxPos; i++)
      {
        ...
        arrCatTxt[i] = new SFTextField(pnlMain,340,35 + (i * 30),290,25,"XXX");
    ...
    private function onCreateClick(e:MouseEvent):void
    {
      ...
      var xml:String = "<command><function>updatescri</function>";
...
//      SFScreen.addDebug(response.toXMLString());
      ...
      else
      {
        ...
        if(currentCommand == "getsceneinfo")
        {
          ...
          for each (scrientry in scri.scrientry)
          {
            currCat = scrientry.@category;
            currKeychar = scrientry.@keychar;
            currPosition = scrientry.@position;

            pos = parseInt(currPosition);
            pos--;

            arrCatTxt[pos].setText(currCat);
