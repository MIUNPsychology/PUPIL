package pupil.teacher
{
  ...
  public class EditOptionStaticImage
  {
    ...
    public function EditOptionStaticImage(myscreen:SFScreen, parentwin:ProjectDetails, pattern:String, scenename:String, projname:String)
    {
      screen = myscreen;
      myparent = parentwin;
...
//      SFScreen.addDebug("optclick()");
    ...
    private function onCreateClick(e:MouseEvent):void
    {
      ...
      var xml:String = "<command><function>updatesosi</function>";
...
//      SFScreen.addDebug(response.toXMLString());
      ...
      else
      {
        ...
        if(currentCommand == "getsceneinfo")
        {
          ...
          pos = 0;
          for each (sosientry in sosi.sosiopt)
          {
            pos++;
          }
          ...
          for each (sosientry in sosi.sosiimg)
          {
            img = sosientry.@image;
            cat = sosientry.@category;
            currPosition = sosientry.@position;
          ...
          for each (sosientry in sosi.sosiopt)
          {
            currKeychar = sosientry.@keychar;
            currOption = sosientry.@optiontxt;
